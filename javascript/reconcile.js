...
export default (rootElement, newState, keyAttribute) => {
  ...
  const onBeforeElUpdated = (fromElement, toElement) => {
    ...
    // Add support for maintaining AlpineJS state when updating a DOM element
    //
    // If the element we're replacing has an Alpine state copy that state onto
    // the new element before it gets inserted
    if (fromElement.__x) {
      window.Alpine.clone(fromElement.__x, toElement);
    }

    // Take the new version of the element
    return true;
  };
